...
class VideoFeat:
    ...
    def realTimeVideo(self):
        ...
        videoWriter = cv2.VideoWriter('./record_video/douyin'+str(time.time())+'.mp4', cv2.VideoWriter_fourcc(*'H264'), 15, (frame_w,frame_h))
        ...
        while True:
            ...
            if record_status:
                ...
                if time.time() - triger_time >= 1:
                    ...
                    if frame_count < 40:
                        ...
                        # > 50，停止，预测
                        last_action,dist_list = self.calSimilarity(seq_feats)
                        ...
                        # 发送MQTT信息
                        # self.ha_mqtt.sendRemoteCommand(last_action)
            ...
            # 显示
            for index,(y,x,score) in enumerate(results):
                x = int(x * frame_w)
                y = int(y * frame_h)
            ...
            cv2.putText(frame,text,(50,160),cv2.FONT_ITALIC,1,(0,255,0),2)
            ...
            cv2.putText(frame, 'FPS: '+ str(fps_txt), (50,120), cv2.FONT_ITALIC, 1, (0,255,0),2)

            # 左下角信息
            if last_action != '...':
                for index,(name,dist) in enumerate(dist_list):
                    text = '{} {} {}'.format(index+1,name,round(dist,2))
                    cv2.putText(frame, text, (50,(350+30*index)), cv2.FONT_ITALIC, 0.7, (255,0,255),2)
